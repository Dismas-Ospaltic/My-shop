...
interface InventoryUpdateDao {
    ...
    @Query("SELECT * FROM inventory_update ORDER BY timestamp DESC")
    fun getAllInventoryUpdates(): Flow<List<InventoryUpdateEntity>>

    @Query("SELECT * FROM inventory_update WHERE productId = :productId ORDER BY timestamp DESC")
    fun getInventoryUpdatesByProductId(productId: String): Flow<List<InventoryUpdateEntity>>

    @Query("SELECT * FROM inventory_update WHERE date = :date ORDER BY timestamp DESC")
